...
| # | Item |
|---|------|
| SOC-Reels | ✅ SHIPPED 2026-07-18 (PR #19, task-def rev 48) — daily 9:16 reel with podcast audio + burned-in captions via `mc_video.py`, IG Graph Reels flow (`media_type=REELS`) + best-effort X chunked video. Gated by `SOCIAL_MEDIA_KIND` (default `card`; flip to `reel` to enable daily). Live-verified: instagram.com/reel/Da9YFyqCrzQ/. |
| SOC-Reel-Audio | **Fix reel audio/teaser production.** The cold-open selection in `mc_video._select_cold_open`/`build_daily_reel` uses WHOLE segments only, so reel length is uneven — a run came out **15s (host intro only)** because segment 2 was 353 words (~140s) and got dropped by the 88s cap. Fix: truncate an over-long segment to its first N sentences (map sentence boundaries → proportional audio) instead of dropping it, so every reel is ~60–75s of real *story* content, not just Mike's intro. Optionally an LLM-picked highlight window instead of the literal cold-open. |
| SOC-X-Video-Card | **Give every X post a visual (card-image fallback).** `mc_social.upload_video_to_x` already attaches the reel to the tweet, but it's best-effort (free-tier ~17/day INIT/FINALIZE cap) and currently falls back to **bare text+link** when video upload is unavailable. Add a static-card fallback: upload the 1080×1080 card (`generate_card`) as tweet media so the X post always carries an image even when the video can't post. (Was noted as the optional fallback in `SOCIAL_VIDEO_PLAN.md` §3/§7.) |
...
| SOC-Clips | Daily short-form clips with burned-in captions (batch 30–45) — new post-production Fargate stage + one Docker image (ffmpeg + Whisper deps), triggered after the episode publishes. (1) **Segment selection:** new CrewAI agent (mirror `make_social_copywriter()`/`distribution_crew.py`) picks the best self-contained 30–60s hook — select on the pre-TTS script text and map to audio timestamps (cheaper than Whisper-transcribing the rendered audio; MyAIGist has Whisper if audio-first is preferred). (2) **Render:** `ffmpeg` cuts the segment, composes over the branded card (`mc_social.generate_card`/`mc_ad.py`) or an `ffmpeg showwaves` waveform. (3) **Captions (non-negotiable — short-form watched muted):** Whisper word-level timestamps → styled SRT/ASS burned in via ffmpeg; use `stable-ts`/`whisperX` for cleaner timing. (4) **Publish:** YouTube Shorts only first (YT Data API from SOC-YT), validate segment quality ~1 week, then IG Reels (overlaps SOC-Reels) then TikTok; per-platform `channel_sent` gate. |
| SOC-Bio | Credibility layer — tight two-line bio everywhere the show appears (podcast/YouTube descriptions, episode outro, site header): *"From Mike Schwimmer — 14 years at Google, most recently Director of Customer Solutions Engineering and co-founder of the Ads AI Task Force. Two ad tech patents."* Plus a ~20s AI-voiced credential line in the intro/outro template and an `/about` page doubling as a future white-label sales page. Converts clip-driven clicks into subscribes. |
