...
| SOC-Reel-Audio | ✅ SHIPPED 2026-07-25 (PR #22, task-def rev 51) — `_select_cold_open` now truncates an over-long cold-open segment to its first whole sentences (`_truncate_to_words`/`_split_sentences`) instead of dropping it, so the reel always carries real story content. Verified: 2026-07-18 went 15s (intro only) → 58.4s. *(Future option: an LLM-picked highlight window instead of the literal cold-open.)* |
